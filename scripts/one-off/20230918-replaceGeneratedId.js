...
var kebabify = function (input, keepCase) {
...
function createHeaderAnchor(header) {
    var regex = /^[\d.]+\s/;
    return header.match(regex) ? kebabify(header.replace(regex, "")) : kebabify(header);
}
var contentPath = (0, path_1.join)(__dirname, "../../content");
var addAnchorsToLinks = function (anchor) {
    var hrefWithLanguage = anchor[1], generatedAnchor = anchor[2], customAnchor = anchor[3];
    var href = hrefWithLanguage.replace(/\/(en|fr)\//, "/");
    glob("**/*.md", { cwd: contentPath }, function (err, files) {
        if (err) {
            console.error("Error while reading files:", err);
            return;
        }
        files.forEach(function (file) {
            var path = (0, path_1.join)(contentPath, file);
            var fileContent = (0, fs_1.readFileSync)(path, "utf-8");
            var regex = new RegExp("\\(".concat(href, "#").concat(generatedAnchor, "\\)"), "g");
            fileContent = fileContent.replace(regex, "(".concat(href, "#").concat(customAnchor, ")"));
            (0, fs_1.writeFileSync)(path, fileContent, "utf-8");
        });
    });
};
var addAnchorToHeading = function (anchor) {
    var relativePath = anchor[0], generatedAnchor = anchor[2], customAnchor = anchor[3];
    var filePath = (0, path_1.join)(contentPath, relativePath);
    if (!(0, fs_1.existsSync)(filePath)) {
        console.warn("File ".concat(filePath, " does not exist"));
        return;
    }
    var fileContent = (0, fs_1.readFileSync)(filePath, "utf-8");
    var isTitle = function (line) {
        var match = line.match(/^#{2,4} (.+)/);
...
anchors.forEach(function (anchor) { return addAnchorToHeading(anchor); });
anchors.forEach(function (anchor) { return addAnchorsToLinks(anchor); });
